...
class Employee:
    ...
    def to_dict(self):
        return {'id': self.id, 'name': self.name, 'job_title': self.job_title, 'salary': self.salary}

    @staticmethod
    def from_dict(employee_dict):
        return Employee(employee_dict['id'], employee_dict['name'], employee_dict['job_title'], employee_dict['salary'])
