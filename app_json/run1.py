...
while True:
  ...
  if choice == 1:
    ...
    print("Enter Salary:")
    salary = input()
    repo1.add_employee(Employee(id=id, name=name, job_title=title, salary=salary))
  elif choice == 2:
    print("Enter id to search:")
    id = int(input())
    index = repo1.search_employee(id)
    if index != -1:
       print("Searched Employee: ", repo1.employees[index])
    else:
    ...
    index = repo1.search_employee(id)
    if index != -1:
        repo1.employees.pop(index)
        print("Employee deleted successfully")
    else:
        print("Employee not found")
  elif choice == 5:
    print(repo1.employees)
  elif choice == 6:
